...
class RevSRec(SASRec):
    def __init__(self, config, dataset):
        ...
        self.trm_encode = TransformerEncoder(n_layers=self.n_layers, n_heads=self.n_heads, \
        ...
        file_path = '/mnt/sunwenqi/UniSRec/dataset/raw/Reviews/user_emb.npy'
        ...
        pretrained_user_embedding = torch.from_numpy(embedding_matrix).float()
        pretrained_user_embedding.to(self.device)
        print('device')
        print(self.device)
        self.user_emb = nn.Embedding.from_pretrained(pretrained_user_embedding, freeze=config['freeze_embedding'])
        self.user_emb.to(self.device)

        print('user_emb:')
        print(self.user_emb)
        print(self.user_emb.weight)
        ...
        #self.zero = torch.zeros([dataset.user_num, 300], dtype=torch.float).to(self.device)
        #print('users:')
        #print(self.n_users)

    def forward(self, user_id, item_seq, item_emb, item_seq_len):
        ...
        #user_id = torch.where(user_id>=self.n_users, torch.zeros(user_id.shape,dtype=torch.long).to(self.device), user_id)
        #print('user emb:')
        #print('user id:')
        #print(user_id)
        #print(user_id.shape)
        #print(user_id.ge(8441))
        ...
        trm_output = self.trm_encode(user_embedding, input_emb, extended_attention_mask, output_all_encoded_layers=True)
        output = trm_output[-1]
    ...
    def calculate_loss(self, interaction):
        ...
        #print('item seq')
        #print(item_seq[0:1])
        seq_output = self.forward(user_id, item_seq, item_emb_list, item_seq_len)
